...
// --- Seus dados ---
#define NOME_ALUNO "Lucas Zucoloto Soares Valbusa"
#define RM_ALUNO "86935"
// ------------------
...
void vTaskSupervisor(void *pvParameters) {
    ...
    while (1) {
        ...
        printf("{%s-RM:%s} [SUPERVISAO] Status: Gerador [%s], Receptor [%s]\n", NOME_ALUNO, RM_ALUNO, flagGeracaoOk ? "ATIVO" : "INATIVO", flagRecepcaoOk ? "ATIVO" : "INATIVO");
        flagGeracaoOk = false;
...
void app_main(void) {
    printf("{Lucas Zucoloto Soares Valbusa-RM:86935} [SISTEMA] Iniciando CP2 - Sistema de Dados Robusto.\n");
    xFilaDeDados = xQueueCreate(TAMANHO_FILA, sizeof(int));
    if (xFilaDeDados == NULL) {
        printf("{Lucas Zucoloto Soares Valbusa-RM:86935} [SISTEMA] Falha ao criar a fila.\n");
        return;
    }
    ...
    printf("{Lucas Zucoloto Soares Valbusa-RM:86935} [SISTEMA] Inicializacao concluida. WDT ja ativo.\n");

    // Loop infinito para manter a app_main viva
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(10000)); // Apenas "pausa" para economizar energia
    }
}